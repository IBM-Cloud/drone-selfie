...
public class DatabaseUtilities {
	...
	public CouchDbConnector getDB() {
		...
		try {
			String db_url = System.getenv("DS_DB_HOST");
			String db_user = System.getenv("DS_DB_USERNAME");
			String db_pw = System.getenv("DS_DB_PASSWORD");	
			...
			if (configExists) {
				...
				httpClient = new StdHttpClient.Builder()
						.url("https://" + host + "/drone-selfie")
						.username(username).password(password).build();

				CouchDbInstance dbInstance = new StdCouchDbInstance(httpClient);

				db = dbInstance.createConnector("drone-selfie", true);
